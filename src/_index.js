...
const apiMap = {
  ...
  mobileQQ: '//open.mobile.qq.com/sdk/qqapi.js?_bid=152'
};
...
// 初始化微信分享配置(在微信中打开调用)
const _initWeixinShareConfig = () => {
  // 需要先完成微信认证,再进行配置，参考https://mp.weixin.qq.com/wiki?t=resource/res_main&id=mp1421141115
};

// 初始化qq分享配置(qq中打开调用)
const _initQQShareConfig = (data) => {
  if (util.ua.isFromQQ && (window.mqq && mqq.data && mqq.data.setShareInfo)) {
    util.loadScript(apiMap.mobileQQ, () => {
